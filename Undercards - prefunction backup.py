...
#Make GUI and design an actual thing
...
for x in range(26): #Gives player 1 26 random non-summon-only cards
    ...
    while player1["deck"][len(player1["deck"])-1]["rarity"] == "X": #Replaces card every time a summon-only card is given to you
        player1["deck"].remove(player1["deck"][len(player1["deck"])-1])
        player1["deck"].append(cards[random.randint(0,len(cards)-1)])
...
for x in range(3): #3 times
    ...
    while player1["deck"][len(player1["deck"])-1]["rarity"] == "X": #Replaces card every time a summon-only card is given to you
            player1["hand"].remove(player1["hand"][len(player1["hand"])-1])
            player1["hand"].append(cards[random.randint(0,len(cards)-1)])
    ...
    if answer1 == "yes":
        player1["hand"].remove(player1["hand"][len(player1["hand"])-1]) #Removes latest card given (this one)
        player1["hand"].append(cards[random.randint(0, len(cards)-1)]) #Gives you another random card
        while player1["deck"][len(player1["deck"])-1]["rarity"] == "X": #Replaces card every time a summon-only card is given to you
            player1["hand"].remove(player1["hand"][len(player1["hand"])-1])
            player1["hand"].append(cards[random.randint(0,len(cards)-1)])
        print("Card swapped!")
...
for x in range(3): #3 times
    ...
    answer1 = input("Would you like to swap it for a different random card? Type 'yes' to do so, any other response will mean your card is kept: ")
    if answer1 == "yes":
        player2["hand"].remove(player2["hand"][len(player2["hand"])-1]) #Removes latest card given (this one)
        player2["hand"].append(cards[random.randint(0, len(cards)-1)]) #Gives you another random card
        while player1["deck"][len(player1["deck"])-1]["rarity"] == "X": #Replaces card every time a summon-only card is given to you
            player1["hand"].remove(player1["hand"][len(player1["hand"])-1])
            player1["hand"].append(cards[random.randint(0,len(cards)-1)])
        print("Card swapped!")
...
def printBoard():
    ...
    for x in range(0,8):    #1: Name | ATK:6 | HP:8/9 | Cost:7 | CAN ATK: False | TAUNT | ARMOUR:2 | CAN'T ATTACK! | CHARGE | DODGE:2 | HASTE | RANGED
        if board[x] == None:
            print(str(x+1)+":", "Empty")
        else:
                    ...
                    while invalidAns2 == True:     #each card in your hand
                        ...
                        if space == "1" and board[0] == None: #The comments for this if apply to all the below ones
                            if currentPlayer["gold"] >= x["cost"]: #Checks if you have enough gold
                                board[0] = copy.deepcopy(x) #Copies the stats for the selected card to the selected space but leaves the actual card stats as they are.
                                ...
                                print("You now have", str(currentPlayer["gold"]), "gold.")
                            else:
                            ...
                            if currentPlayer["gold"] >= x["cost"]:
                                board[1] = copy.deepcopy(x) #Copies the stats for the selected card to the selected space but leaves the actual card stats as they are
                                currentPlayer["hand"].remove(x)
                                invalidAns2 = False
                                invalidAns = False
                                currentPlayer["gold"] -= x["cost"]
                            ...
                            if currentPlayer["gold"] >= x["cost"]:
                                board[2] = copy.deepcopy(x) #Copies the stats for the selected card to the selected space but leaves the actual card stats as they are
                                currentPlayer["hand"].remove(x)
                                invalidAns2 = False
                                invalidAns = False
                                currentPlayer["gold"] -= x["cost"]
                            ...
                            if currentPlayer["gold"] >= x["cost"]:
                                board[3] = copy.deepcopy(x) #Copies the stats for the selected card to the selected space but leaves the actual card stats as they are
                                currentPlayer["hand"].remove(x)
                                invalidAns2 = False
                                invalidAns = False
                                currentPlayer["gold"] -= x["cost"]
                ...
                if card == x["name"]:         #answer against
                    while invalidAns2 == True:     #each card in your hand
                        ...
                        if space == "5" and board[4] == None: #The comments for this if apply to all the below ones
                            if currentPlayer["gold"] >= x["cost"]: #Checks if you have enough gold
                                board[4] = copy.deepcopy(x) #Copies the stats for the selected card to the selected space but leaves the actual card stats as they are
                                currentPlayer["hand"].remove(x) #Removes the card from your hand
                                invalidAns2 = False
                                invalidAns = False
                                currentPlayer["gold"] -= x["cost"] #Takes the card cost away from your gold
                            ...
                            if currentPlayer["gold"] >= x["cost"]:
                                board[5] = copy.deepcopy(x) #Copies the stats for the selected card to the selected space but leaves the actual card stats as they are
                                currentPlayer["hand"].remove(x)
                                invalidAns2 = False
                                invalidAns = False
                                currentPlayer["gold"] -= x["cost"]
                            ...
                            if currentPlayer["gold"] >= x["cost"]:
                                board[6] = copy.deepcopy(x) #Copies the stats for the selected card to the selected space but leaves the actual card stats as they are
                                currentPlayer["hand"].remove(x)
                                invalidAns2 = False
                                invalidAns = False
                                currentPlayer["gold"] -= x["cost"]
                            ...
                            if currentPlayer["gold"] >= x["cost"]:
                                board[7] = copy.deepcopy(x) #Copies the stats for the selected card to the selected space but leaves the actual card stats as they are
                                currentPlayer["hand"].remove(x)
                                invalidAns2 = False
                                invalidAns = False
                                currentPlayer["gold"] -= x["cost"]
                    ...
                    if board[x]["name"] == atkCard and board[x]["canAttack"] == True:
                        ...
                        while invalidAns2 == True:
                            ...
                            for y in range(4,8): #Had to be filled with [invalidAns = True]s to avoid being executed one more time when exiting the whole loop
                                if board[y] != None: #Check: 1
                                    if target == board[y]["name"] and invalidAns == True: # \/ this checks if there are taunt monsters on the enemy's board << 3
    ...
    for x in range(0,8): #Any cards with less than one HP are declared dead, board space is cleared
        if board[x] != None:
            if "charge" in board[x].keys() and board[x]["charge"] == True: #The card can attack on the first turn
                ...
                pass
            if "haste" in board[x].keys() and board[x]["haste"] == True:
                board[x]["canAttack"] = True #Allows attacking, stopping player attacks is embedded within attack command
            else:
                pass
            if board[x]["health"] < 1:
